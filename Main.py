...
def dashboard():
    ...
    # 🔍 NUEVO: Buscar EXCLUSIVAMENTE el ciclo donde el usuario es Nivel 5 (su ciclo de origen)
    cur.execute("""
    ...
    my_sales_history = []
    income_history = []
    # 🔍 NUEVO: Filtrar historial SOLO al ciclo de origen (donde es L5)
    if cycle_id:
    ...
    # ✅ Historial global de transferencias confirmadas cuando actuó como Nivel 1
    cur.execute("""
...
def crear_sticker():
    ...
    try:
        ...
        # Registrar al vendedor como Nivel 5 en ESTE ciclo
        ...
        # Asignar niveles 4,3,2,1 subiendo por referral_tree hasta ADMIN001
        ...
        # Verificar si ya hay un sticker pendiente en ESTE ciclo
        ...
        # 🔍 NUEVO: Eliminado el bloqueo global por ventas >=3
        # Cada ciclo es independiente, no se bloquea por ventas en otros ciclos
        step = 1  # Nuevo sticker en nuevo ciclo siempre empieza en paso 1
...
def enviar_datos_email(sticker_id):
    ...
    try:
        ...
        if s and s["status"] == "confirmed":
            ...
            # 🔍 NUEVO: Conteo de ventas ENTREGADAS SOLO en ESTE ciclo específico
            ...
            if entregados == 3:
                ...
                # 2. Bajar exactamente 1 nivel al resto (Niveles 2, 3, 4, 5) SOLO en ESTE ciclo
