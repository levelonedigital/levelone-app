...
def dashboard():
    ...
    u = dict(row_u); uid = u.get("id"); role = u.get("role", "seller")
    sticker = u.get("sticker_id", ""); level = u.get("current_level", 5)

    if level == 5:
        cur.execute("SELECT * FROM cycles WHERE l5_user_id=%s", (uid,)); cycles = cur.fetchall()
    else:
        cur.execute("SELECT * FROM cycles WHERE l5_user_id=%s OR id IN (SELECT cycle_id FROM cycle_levels WHERE user_id=%s)", (uid, uid)); cycles = cur.fetchall()
        
    cycle_id = request.args.get("cycle_id", type=int)
    if not cycle_id and cycles: cycle_id = cycles[-1]["id"]
    active_cycle = None
    if cycle_id:
        cur.execute("SELECT * FROM cycles WHERE id=%s", (cycle_id,)); active_cycle = cur.fetchone()
